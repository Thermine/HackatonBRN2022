    ...
    public class HVRRayCastGun : HVRDamageProvider
    {
        ...
        public virtual void ReleaseAmmo()
        {
            ...
            if (ammoGrabbable)
            {
                ...
                if (hand)
                {
                    foreach (var handCollider in hand.HandColliders)
                    {
                        foreach (var ammoCollider in ammoGrabbable.Colliders)
                        {
                            Physics.IgnoreCollision(handCollider, ammoCollider, true);
                        }
                    }
                }
        ...
        private IEnumerator RenablePhysics(HVRGrabbable grabbable, HVRHandGrabber hand)
        {
            ...
            if (hand)
            {
                foreach (var handCollider in hand.HandColliders)
                {
                    foreach (var ammoCollider in grabbable.Colliders)
                    {
                        Physics.IgnoreCollision(handCollider, ammoCollider, false);
                    }
                }
            }
        }
