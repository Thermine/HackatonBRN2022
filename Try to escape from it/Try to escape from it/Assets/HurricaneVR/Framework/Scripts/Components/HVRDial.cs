    ...
    public class HVRDial : MonoBehaviour
    {
        ...
        protected void CheckForRotation(bool force = false)
        {
            if (force || PrimaryGrabber)
            {
                ...
                var currentVector = transform.TransformDirection(LocalAxisStart);
                var rotationAxis = transform.TransformDirection(LocalRotationAxis);
                ...
                var newVector = rotation * currentVector;
                ...
                var syncRotation = Quaternion.AngleAxis(syncAngle, rotationAxis) * transform.rotation;
                ...
                var steppedRotation = Quaternion.AngleAxis(steppedAngle, rotationAxis) * transform.rotation;

                var targetRotation = SnapTarget ? steppedRotation : syncRotation;

                if (_targetRigidBody)
                {
                    _targetRigidBody.MoveRotation(targetRotation);
                }
                else
                {
                ...
                if (_rigidBody)
                {
                    _rigidBody.MoveRotation(syncRotation);
                }
                else
                {
