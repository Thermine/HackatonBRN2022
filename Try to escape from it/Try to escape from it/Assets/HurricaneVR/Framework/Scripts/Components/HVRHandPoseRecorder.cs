...
namespace HurricaneVR.Framework.Components
{
    public class HVRHandPoseRecorder : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                if (LeftPhysics)
                    LeftPhysics.SetAllToTrigger();

                if (RightPhysics)
                    RightPhysics.SetAllToTrigger();
            }
            else if (!DisablePhysics && _previousDisable)
            {
                if (LeftPhysics)
                    LeftPhysics.ResetToNonTrigger();

                if (RightPhysics)
                    RightPhysics.ResetToNonTrigger();
            }
...
#if UNITY_EDITOR
            var pose = hand.CreateFullHandPose(hand.MirrorAxis, true);
